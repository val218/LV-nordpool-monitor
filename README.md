...
└── src/
    ...
    ├── gt911.cpp
    ├── xl9535.cpp
    └── fonts/              generated by tools/build_fonts.sh — gitignored
        └── np_font_*.c
```

## Custom fonts

The LCD shows English, Latvian, and Russian. The default LVGL Montserrat
font is ASCII-only — Latvian diacritics (ā ē ī ū č š ž etc.) and Cyrillic
characters render as missing-glyph squares.

To fix this, the project uses **Noto Sans** with custom character ranges:

- Basic Latin
- Latin-1 Supplement
- Latin Extended-A (covers Latvian)
- Cyrillic block (covers Russian)
- Common typographic punctuation

The font files (`src/fonts/np_font_*.c`) are **not committed to git** —
they're generated by `tools/build_fonts.sh` either:

- **Locally** if you want to build outside of CI:
  ```bash
  bash tools/build_fonts.sh
  ```
  Requires Node.js ≥ 14, `npm`, and `curl`. The script auto-installs
  `lv_font_conv`. Generated `.c` files end up in `src/fonts/`.

- **Automatically in CI** before each build — see `.github/workflows/main.yml`.

If you change the character ranges or sizes, edit `tools/build_fonts.sh`
and `include/np_fonts.h` to match.
