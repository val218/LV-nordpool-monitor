...
// compiles its generated .c files (the LVGL fonts) into the firmware.
// This is the only purpose of np_fonts_lib.h — the actual font symbol
// declarations live in include/np_fonts.h, which is pulled in by ui.h.
...
// Callback for streaming CSV parse — invoked once per line (legacy fallback).
static bool priceStreamLine(const String& line, void* ctx) {
    PriceData* p = (PriceData*)ctx;
    p->feedLine(line);
...
static bool tryFetchPrices() {
    ...
    // Primary source: e-cena.lv (~7 KB HTML, today + tomorrow only).
    Serial.println("[APP] Fetching prices from e-cena.lv...");
    unsigned long t0 = millis();
    String html;
    if (g_net.fetchEcenaHtml(html)) {
        int n = g_prices.parseEcenaHtml(html);
        Serial.printf("[APP] e-cena fetch+parse OK in %lums, %d entries stored\n",
                      millis() - t0, n);
        if (n > 0) return true;
        Serial.println("[APP] e-cena returned 0 entries, falling back to nordpool");
    } else {
        Serial.printf("[APP] e-cena fetch FAILED after %lums, falling back\n",
                      millis() - t0);
    }

    // Fallback: stream the big nordpool.didnt.work CSV. Slow (~30s) but
    // historically reliable.
    Serial.println("[APP] Fetching prices from nordpool (fallback, streaming)...");
    t0 = millis();
    g_prices.beginStream();
    bool ok = g_net.fetchPricesCsvStream(priceStreamLine, &g_prices);
    g_prices.endStream();
    Serial.printf("[APP] Fallback fetch %s in %lums, %d entries stored\n",
                  ok ? "OK" : "FAILED", millis() - t0, g_prices.count());
    return ok && g_prices.count() > 0;
}
