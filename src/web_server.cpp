...
void WebUI::begin() {
    ...
    _server.on("/api/debug-fetch", HTTP_GET, [this](AsyncWebServerRequest* req) {
        // Synchronous HTTPS fetch for diagnostic purposes. Reports the result
        // back to the browser as plain text so it's easy to read.
        // This blocks the web server briefly, but it's fine for a debug tool.
        String report;
        report.reserve(8192);
        report += "Nordpool fetch diagnostic\n";
        report += "==========================\n\n";

        if (!_net.isConnected()) {
            report += "WiFi: NOT CONNECTED\n";
            report += "Cannot fetch without WiFi.\n";
            req->send(200, "text/plain; charset=utf-8", report);
            return;
        }
        report += "WiFi: connected, IP " + _net.ipString() + "\n";
        report += "URL: ";
        report += NORDPOOL_CSV_URL;
        report += "\n\n";

        unsigned long t0 = millis();
        String csv;
        bool ok = _net.fetchPricesCsv(csv);
        unsigned long elapsed = millis() - t0;

        report += "Fetch result: " + String(ok ? "SUCCESS" : "FAILED") + "\n";
        report += "Time: " + String(elapsed) + " ms\n";
        report += "Bytes received: " + String(csv.length()) + "\n\n";

        if (ok && csv.length() > 0) {
            report += "First 500 characters of response:\n";
            report += "---------------------------------\n";
            report += csv.substring(0, 500);
            report += "\n---------------------------------\n\n";

            // Also try parsing
            extern class PriceData g_prices;
            int n = g_prices.parseCsv(csv);
            report += "Parsed " + String(n) + " price entries\n";
        } else {
            report += "(no data returned)\n";
        }
