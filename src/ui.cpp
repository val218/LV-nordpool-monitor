...
void UI::buildRelays() {
    ...
    for (int i = 0; i < NUM_RELAYS; ++i) {
        ...
        tt.icon = makeLabel(tt.card, "?", COL_TEXT, &lv_font_montserrat_28);
        lv_obj_align(tt.icon, LV_ALIGN_CENTER, 0, -4);

        tt.name = makeLabel(tt.card, "Relay", COL_TEXT, &lv_font_montserrat_12);
        ...
        tt.mode = makeLabel(tt.card, "", COL_DIM, &lv_font_montserrat_12);
        lv_obj_align(tt.mode, LV_ALIGN_BOTTOM_LEFT, 2, -16);

        tt.threshold = makeLabel(tt.card, "", COL_TEXT, &lv_font_montserrat_12);
...
void UI::buildEdit() {
    ...
    _editIconLabel = makeLabel(_editIconBtn, "?", COL_TEXT, &lv_font_montserrat_14);
...
void UI::buildIconPick() {
    ...
    for (int i = 0; i < ICON_COUNT && i < cols * rows; ++i) {
        ...
        lv_obj_t* g = makeLabel(cell, iconText((RelayIcon)i),
                                COL_TEXT, &lv_font_montserrat_24);
        lv_obj_center(g);
    }
}
...
void UI::buildFullChart() {
    ...
    // X-axis hour labels
    _fullXAxis = makeLabel(_scrFullChart, "",
                           COL_DIM, &lv_font_montserrat_12);
    lv_obj_align(_fullXAxis, LV_ALIGN_BOTTOM_MID, 0, -2);
    ...
    lv_obj_set_style_text_font(tipLbl, &lv_font_montserrat_14, 0);
...
void UI::refreshFullChart() {
    ...
    // X-axis: hour labels at 0/6/12/18/24/30/36/42/47
    char xb[80];
    snprintf(xb, sizeof(xb), "0   6   12   18   24   30   36   42   48");
    lv_label_set_text(_fullXAxis, xb);
