...
# Output font files DIRECTLY into src/ (not src/fonts/). PlatformIO's default
# build_src_filter = +<*> picks up every .c at the top of src/ reliably,
# whereas subdirectory recursion has been observed to fail in CI even with
# explicit filters. Files are prefixed np_ so they're easy to gitignore.
